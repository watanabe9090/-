...
class AudioSettingsMenu(Gamable):
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface, WHITE, (x, y, self.volume_width*BLOCK, self.volume_height*BLOCK ), 1, 0)
        surface.fill( WHITE, (x, y, self.volume*BLOCK, self.volume_height*BLOCK))

        y2 = HEIGHT / 2 * BLOCK
        x2 = WIDTH / 2 * BLOCK
        surface.blit(self.back_option, (x2, y2+BLOCK*6))
        if self.optionIndex == 0:
            surface.blit(self.font.render(CURSOR, True, WHITE), (x-BLOCK*7, y+self.volume_height/2*BLOCK))
        if self.optionIndex == 1:
            surface.blit(self.font.render(CURSOR, True, WHITE), (x2-BLOCK*7, y2+BLOCK*6))
